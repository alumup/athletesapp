  ...
  useEffect(() => {
    const fetchTickets = async () => {
      ...
      if (error) {
        ...
        setSponsorships(
          data.filter((product) => product.type === "sponsorship"),
        );
        ...
        data
          ...
          .forEach((ticket, i) => {
            setValue(
              `tickets[${i}].count`,
              initialValues.tickets?.[i]?.count || 0,
            );
          });
        data
          .filter((product) => product.type === "sponsorship")
          .forEach((sponsorship, i) => {
            setValue(
              `sponsorships[${i}].count`,
              initialValues.sponsorships?.[i]?.count || 0,
            );
          });
      }
    };
      ...
      <div className="">
        <div>
          <form onSubmit={handleSubmit(onSubmit)}>
            <p className="text-xl leading-6 text-gray-700 md:text-2xl">
              Tickets
            </p>
            <p className="mb-5 text-sm font-light leading-6 text-gray-500 md:text-base">
              Buy up to 4 event tickets to participate in our 4-man scramble
              golf event.
            </p>
            {tickets.map((ticket, index) => (
              <div
                key={ticket.id}
                className="mt-3 w-full rounded border border-gray-200 p-5"
              >
                <div className="flex items-center justify-between border-b border-gray-200 pb-3">
                  <div>
                    <h5 className="md:text-md text-sm font-bold">
                      {ticket.name}
                    </h5>
            ...
            <h3 className="mt-5 text-xl leading-6 text-gray-700 md:text-2xl">
              Sponsorships
            </h3>
            <p className="mb-5 text-sm font-light leading-6 text-gray-500 md:text-base">
              We need sponsors to help pay for the event so all of the proceeds
              can go towards running Provo High Basketball.
            </p>
              ...
              >
                <div className="flex items-center justify-between border-b border-gray-200 pb-3">
                  <div>
                    ...
                    <p className="md:text-md text-sm">
                      {(sponsorship.price / 100).toLocaleString("en-US", {
                        style: "currency",
                        currency: "USD",
                      })}
                    </p>
                  ...
                  <div>
                    ...
                    <input
                      type="hidden"
                      {...register(`sponsorships[${index}].name`)}
                      value={sponsorship.name}
                    />
                    <input
                      type="hidden"
                      {...register(`sponsorships[${index}].price`)}
                      value={sponsorship.price}
                    />
                    <input
                      type="hidden"
                      {...register(`sponsorships[${index}].id`)}
            ...
            <div className="fixed inset-x-0 bottom-0 flex items-center justify-end border-t  border-gray-200 bg-white p-5 shadow-sm md:relative">
              {!isFirstStep && (
