  ...
  return (
    <div key={id} className={`theme ${data?.theme?.value} bg-background text-foreground relative isolate px-3 md:px-5 py-10 md:py-20`}>
      ...
      <div className="mt-5 flex items-center justify-center space-x-2">
        {data.primaryCta && (
          <a href={data?.primaryCta?.properties?.href.value} className="bg-primary text-primary-foreground shadow px-3 py-2 rounded">{data?.primaryCta?.properties?.text?.value}</a>
        )}
        {data.secondaryCta && (
          <a href={data?.secondaryCta?.properties?.href.value} className="border border-primary text-primary px-3 py-2 rounded">{data?.secondaryCta?.properties?.text?.value}</a>
        )}
      </div>
    </div>
  );
