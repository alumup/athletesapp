  ...
  return (
    <>
      ...
      <div className="theme mx-auto max-w-screen-2xl px-4">
        ...
        <Suspense>
          <RelatedProducts id={product.id} domain={params.domain} />
        </Suspense>
      </div>
    </>
  );
...
async function RelatedProducts({ id, domain }: { id: string; domain: string }) {
  const shopify = await getAccountShopify(domain);

  const relatedProducts = await shopify.getProductRecommendations(id);
